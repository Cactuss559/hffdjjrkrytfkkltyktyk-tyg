...
bot.on("message", message => {
    ...
    switch(args[0].toLowerCase()){
        ...
        case "pub":
            if(cooldown.has(message.author.id)){
                ...
                if(message.channel.name === "cpub"){
                    ...
                    if(message.author.id==="319869890456584212"||message.author.id==="274636300332695562"||message.author.id==="347043099010269184"||message.author.id==="317221808405348364"||message.author.id==="394787892351991808"||message.author.id==="296357024864665605"||message.author.id==="398096736880427018"){
                        var embed = new Discord.RichEmbed()
                            .setColor(embedcolor)
                            .setAuthor("Erreur", bot.user.avatarURL)
        ...
        case "modpub":
            if(message.channel.name === "cpub"){
                if(!message.author.id==="308146369745518602"||!message.author.id==="466535873287028737"|!message.author.id==="422353090667347968"|!message.author.id==="356047419013726208"){
                    var embed = new Discord.RichEmbed()
                        .setColor(embedcolor)
                        .setAuthor("Erreur", bot.user.avatarURL)
